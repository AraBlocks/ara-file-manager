...
ipcMain.on(events.OPEN_AFS, async (_, load) => {
  ...
  try {
    ...
    const afsExplorer = windowManager.openWindow('afsExplorerView')
    afsExplorer.object.on('close', () => {
      closeQueued = true
      try {
        const { files } = store
    ...
    if (!closeQueued) dispatch({ type: events.FEED_CONTENT_VIEWER, load: { ...load, fileList, updateAvailable } })
