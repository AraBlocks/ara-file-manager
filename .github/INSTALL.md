...
Additionally, if you ran a demo app in Electron, it may have left some settings here:

```
C:\Users\Kevin\AppData\Roaming\some-electron-demo-name
```
...
If Windows Update is installing something right now, our later step installing the Visual Studio build tools can fail.
...
[Sublime Text](https://www.sublimetext.com/), while not open source, is also a favorite in the Ara team.
...
for the first time will open a dialog box that asks if you want to install [Xcode](https://en.wikipedia.org/wiki/Xcode) from the App Store, or just get the command line development tools.
...
$ gcc --version
*should output a bunch of stuff including:*
Apple LLVM version 10.0.1 (clang-1001.0.46.3)
```

This should get you [gcc](https://en.wikipedia.org/wiki/GNU_Compiler_Collection), the C compiler that will build the native modules.

We won't actually use the Xcode app to program Ara, but you could use it just as a text editor.

## Install Git (Windows)

Download and install Git on Windows from [git-scm.com](https://git-scm.com/).
Choose the 32 or 64-bit download to match your operating system—probably 64 bits for Windows 10.
Proceed through the setup keeping all the defaults.
From the *Start* menu, run **Git Bash**:

<img src="./../../raw/master/.github/install3.png"/>
...
and as a Node version manager: [nave](https://github.com/isaacs/nave).
...
The software that ran the JavaScript in pages was built into web browsers like [Netscape](https://en.wikipedia.org/wiki/Netscape_Navigator).
...
Thanks to V8 and Node, a previously slow language initially designed to check web form input and power page animations could be used as a systems language in place of [C](https://en.wikipedia.org/wiki/C_(programming_language)).
...
He imagined a text editor called [Atom](https://atom.io/), which users could customize with JavaScript as easily as they script a web page.
...
On Windows, install Node.js from [nodejs.org](https://nodejs.org/).
Instead of getting the most recent version, click [Other Downloads](https://nodejs.org/en/download/) and [Previous Releases](https://nodejs.org/en/download/releases/) to find **Node 8.16.0**:
...
We do need the tools necessary to compile native modules, but will get them next a different way that is more likely to complete successfully.

<img src="./../../raw/master/.github/install4.png"/>
...
Not running any container app, Electron reports its versions of Chromium, Node, and V8.

<img src="./../../raw/master/.github/install5.png"/>
...
Instead, installing older versions of Node over newer ones seems to work.
...
`npm install` built the native modules using Node, but then `start-dev` ran Electron, and Electron's newer V8 doesn't like them.
...
This runs at the end of every `npm install` (which previously built the native modules for Node).
You can also do this manually with a command like this:
...
Most of the code in the Ara File Manager and the Node modules it brings along is JavaScript, but there is some [C](https://en.wikipedia.org/wiki/C_(programming_language)) in there.
...
*node-gyp* doesn't actually compile anything, it just scripts a C compiler already installed on your computer to get the job done.
...
On Windows we need to install Visual Studio build tools to get a C compiler.
...
Avoiding the combinations that end *ISE* or *(x86)* to pick the regular one, choose **Run as Administrator** to get this:

<img src="./../../raw/master/.github/install6.png"/>
...
<img src="./../../raw/master/.github/install7.png"/>
...
It'll take up 600 megabytes and consist of about 30 thousand files.
If installing is taking a really long time without producing output, try `npm install -verbose`.
With verbose turned on, you'll see each file npm is downloading.
If you're encountering errors, though, it's easier to see them with verbose turned off.
...
<img src="./../../raw/master/.github/install8.png"/>
